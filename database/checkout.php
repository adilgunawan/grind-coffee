...
echo("<br> ------------------- <br>");
foreach ($data_checkout as $item_checkout) {
    var_dump($item_checkout['cart_id']);
    echo("helow");
...
// // Get the newest cart_id for the user
// $latest_checkout_query = mysqli_query($koneksi, "SELECT cart_id FROM checkout WHERE user_id = '$user_id' ORDER BY tanggal_order DESC LIMIT 1");
// $latest_checkout_result = mysqli_fetch_assoc($latest_checkout_query);
// // var_dump($latest_checkout_result);
// // die();
// $latest_cart_id = $latest_checkout_result['cart_id'];

// // Update only the newest record in the checkout table with the order ID
// $update_order_id_query = mysqli_query($koneksi, "UPDATE checkout SET order_id = '$order_id' WHERE user_id = '$user_id' AND cart_id = '$latest_cart_id'");
